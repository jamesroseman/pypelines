...
kubectl apply -f pypelines/deployment/downloads/k8s/strimzi-cluster-operator-$STRIMZI_VERSION.yaml &&

# Verify Strimzi operator
kubectl get deploy,rs,po &&

# Deploy Kafka Cluster
kubectl apply -f pypelines/deployment/k8s/kafka-cluster.yaml &&

# Deploy Kafka UI
kubectl apply -f pypelines/deployment/k8s/kafka-ui.yaml &&
